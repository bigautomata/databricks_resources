...
To analyze compute utilization (CPU, memory, network), node types (hardware information), cluster compute configurations, and actual cost for running DLT/Lakeflow Spark Declarative Pipelines for "yesterday" (system date minus 1 day) in Databricks, and only for completed pipeline runs, you can use the following SQL query. This query assumes you have access to the relevant system tables and that the necessary columns exist as described.
...
SELECT
  ...
  MAX(n.cpu_user_percent + n.cpu_system_percent) AS peak_cpu_utilization,
  AVG(n.mem_used_percent) AS avg_memory_utilization,
  MAX(n.mem_used_percent) AS peak_memory_utilization,
  AVG(n.network_received_bytes) / (1024 * 1024) AS avg_network_mb_received,
  AVG(n.network_sent_bytes) / (1024 * 1024) AS avg_network_mb_sent,
  SUM(b.usage_quantity) AS total_usage_quantity,
  SUM(b.usage_amount) AS total_cost
FROM system.lakeflow.pipeline_update_timeline p
JOIN system.compute.clusters c
...
LEFT JOIN system.billing.usage b
  ON c.cluster_id = b.cluster_id
     AND DATE(b.usage_start_time) = DATE(p.period_start_time)
WHERE p.result_state = 'COMPLETED'
  AND DATE(p.period_start_time) = DATE_ADD(CURRENT_DATE(), -1)
GROUP BY
  p.workspace_id,
...
```
This query:
- Filters for completed pipeline updates that started "yesterday"
- Joins with clusters for configuration details
- Joins with node timeline for compute utilization metrics
- Joins with node types for hardware specs
- Joins with billing usage for cost
- Aggregates utilization and cost metrics per pipeline update and cluster

**Note:**  
- Adjust column names if your schema differs.
- Ensure you have the necessary permissions to access these system tables.
- The actual column names for hardware specs and billing may vary; check your system tables for exact names.

Let me know if you need a more granular breakdown (e.g., per node or per task) or want to filter for specific pipeline types!

*Always review the accuracy of responses.*

To analyze per-pipeline-run compute utilization, node types, cluster configurations, and actual cost for **Lakeflow Declarative Pipelines** (DLT) for "yesterday" (system date minus 1 day), and to ensure all required columns are included, you should join the following system tables:
...
Below is a comprehensive SQL query that demonstrates how to join these tables, filter for completed pipeline runs for "yesterday", and select the required columns. Adjust the query as needed for your environment and data volume.
...
SELECT
  ...
  p.configuration AS pipeline_configuration,
  ...
  c.cluster_name,
  c.cluster_id,
  c.workspace_id AS cluster_workspace_id,
  c.account_id AS cluster_account_id,

  -- Node type hardware info (worker)
  nt.node_type AS worker_node_type_name,
  nt.core_count AS worker_core_count,
  nt.memory_mb AS worker_memory_mb,

  -- Node timeline (utilization)
  ntl.cluster_id AS node_cluster_id,
  ntl.instance_id,
  ntl.start_time AS node_start_time,
  ntl.end_time AS node_end_time,
  ntl.driver,
  ...
  ntl.network_sent_bytes,
  ntl.network_received_bytes,
  ntl.node_type AS node_type_name,

  -- Billing usage
  bu.account_id AS billing_account_id,
  bu.workspace_id AS billing_workspace_id,
  bu.sku_name,
  bu.usage_start_time,
  bu.usage_end_time,
  bu.usage_date,
  bu.usage_quantity,
  bu.usage_metadata['cluster_id'] AS usage_cluster_id,
  bu.usage_metadata['dlt_pipeline_id'] AS usage_pipeline_id,
  bu.usage_metadata['dlt_update_id'] AS usage_update_id,
  bu.ingestion_date,
  ...
  lp.price_start_time,
  lp.price_end_time,
  lp.account_id AS price_account_id,
  lp.usage_unit,
  lp.pricing['default'] AS default_price

FROM system.lakeflow.pipeline_update_timeline pu
JOIN system.lakeflow.pipelines p
  ON pu.workspace_id = p.workspace_id
  AND pu.pipeline_id = p.pipeline_id
JOIN system.compute.clusters c
  ON pu.compute = c.cluster_id
JOIN system.compute.node_timeline ntl
  ON c.cluster_id = ntl.cluster_id
  AND DATE(ntl.start_time) = DATE(pu.period_start_time)
JOIN system.compute.node_types nt
  ON c.worker_node_type = nt.node_type
LEFT JOIN system.billing.usage bu
  ON c.cluster_id = bu.usage_metadata['cluster_id']
  AND pu.pipeline_id = bu.usage_metadata['dlt_pipeline_id']
  AND pu.update_id = bu.usage_metadata['dlt_update_id']
  AND bu.billing_origin_product = 'DLT'
  AND DATE(bu.usage_date) = DATE(pu.period_start_time)
LEFT JOIN system.billing.list_prices lp
  ON bu.account_id = lp.account_id
  AND bu.usage_type = lp.usage_unit
  AND bu.usage_start_time BETWEEN lp.price_start_time AND lp.price_end_time

WHERE
  pu.result_state = 'COMPLETED'
  AND DATE(pu.period_start_time) = DATE_ADD(CURRENT_DATE(), -1)
  AND p.pipeline_type = 'ETL_PIPELINE'
ORDER BY
  pu.workspace_id, pu.pipeline_id, pu.update_id, ntl.instance_id
```
[[1]](https://docs.databricks.com/aws/en/admin/system-tables/compute/ "/docs.databricks.com/aws/en/admin/system-tables/compute/")

**Key points:**
- This query ensures all required columns from each table are included.
- It filters for completed pipeline runs for "yesterday" and only for ETL_PIPELINE type pipelines.
- Joins are made on relevant keys (e.g., cluster_id, pipeline_id, update_id).
- Billing and pricing are joined to provide cost context per pipeline run.
- You may need to adjust join conditions or column names if your schema differs.

**Limitations:**
- The query assumes that the `compute` field in `pipeline_update_timeline` maps to `cluster_id` in `clusters`.
- If you have multiple node types per cluster (e.g., different worker/driver types), you may need to join `node_types` for both.
- Some records may be missing if clusters or nodes were deleted before October 23, 2023, or if nodes ran for less than 10 minutes[[1]](https://docs.databricks.com/aws/en/admin/system-tables/compute/ "/docs.databricks.com/aws/en/admin/system-tables/compute/").

Let me know if you need aggregation (e.g., average CPU per pipeline run) or further breakdowns!


*Always review the accuracy of responses.*

To provide per-pipeline-run aggregations for Lakeflow Spark Declarative Pipelines (DLT) for "yesterday"—including network, actual used vs unused total memory and CPU, max, min, average memory and CPU usages, and cost per pipeline run—you would use the relevant system tables and aggregate the metrics accordingly.

Below is an example SQL query that demonstrates how to join the required tables and compute the requested aggregations. This query assumes the column names and relationships as you specified:
...
SELECT
  ...
  nt.memory_mb,
  -- CPU aggregations
  MIN(ntl.cpu_user_percent + ntl.cpu_system_percent) AS min_cpu_used_percent,
  MAX(ntl.cpu_user_percent + ntl.cpu_system_percent) AS max_cpu_used_percent,
  AVG(ntl.cpu_user_percent + ntl.cpu_system_percent) AS avg_cpu_used_percent,
  100 - MAX(ntl.cpu_user_percent + ntl.cpu_system_percent) AS min_cpu_unused_percent,
  100 - MIN(ntl.cpu_user_percent + ntl.cpu_system_percent) AS max_cpu_unused_percent,
  100 - AVG(ntl.cpu_user_percent + ntl.cpu_system_percent) AS avg_cpu_unused_percent,
  -- Memory aggregations
  MIN(ntl.mem_used_percent) AS min_mem_used_percent,
  MAX(ntl.mem_used_percent) AS max_mem_used_percent,
  AVG(ntl.mem_used_percent) AS avg_mem_used_percent,
  100 - MAX(ntl.mem_used_percent) AS min_mem_unused_percent,
  100 - MIN(ntl.mem_used_percent) AS max_mem_unused_percent,
  100 - AVG(ntl.mem_used_percent) AS avg_mem_unused_percent,
  -- Network aggregations
  SUM(ntl.network_sent_bytes) / (1024 * 1024) AS total_network_sent_mb,
  SUM(ntl.network_received_bytes) / (1024 * 1024) AS total_network_received_mb,
  -- Cost aggregations
  SUM(bu.usage_amount) AS total_cost
FROM system.lakeflow.pipeline_update_timeline pu
JOIN system.lakeflow.pipelines p
  ON pu.workspace_id = p.workspace_id
  AND pu.pipeline_id = p.pipeline_id
...
ORDER BY
  total_cost DESC
```
[[1]](https://docs.databricks.com/aws/en/data-engineering/observability-best-practices/ "/docs.databricks.com/aws/en/data-engineering/observability-best-practices/")[[2]](https://docs.databricks.com/aws/en/compute/cluster-metrics/ "/docs.databricks.com/aws/en/compute/cluster-metrics/")

**Explanation:**
- **CPU and Memory:** The query calculates min, max, and average used and unused percentages for both CPU and memory.
- **Network:** Sums the total sent and received bytes, converting to MB.
- **Cost:** Sums the usage amount from billing for each pipeline run.
- **Grouping:** All aggregations are per pipeline run (by workspace, pipeline, update, and cluster).

This approach gives you a comprehensive, per-pipeline-run view of resource utilization and cost, supporting observability and optimization for your Lakeflow pipelines[[1]](https://docs.databricks.com/aws/en/data-engineering/observability-best-practices/ "/docs.databricks.com/aws/en/data-engineering/observability-best-practices/")[[2]](https://docs.databricks.com/aws/en/compute/cluster-metrics/ "/docs.databricks.com/aws/en/compute/cluster-metrics/").
*Always review the accuracy of responses.*

Your SQL query for aggregating per-pipeline-run compute utilization, node types, cluster configurations, and cost for Lakeflow Spark Declarative Pipelines is well-structured and aligns with Databricks best practices in several key areas:

### Alignment with Databricks Best Practices

**1. Data Modeling and Joins**  
- The query uses explicit joins between system tables based on well-defined keys (e.g., `pipeline_id`, `update_id`, `cluster_id`), which is recommended for clarity and performance.
- It aggregates at the correct granularity (per pipeline run), which is important for observability and cost analysis[[1]](https://docs.databricks.com/aws/en/data-engineering/best-practices/ "/docs.databricks.com/aws/en/data-engineering/best-practices/").

**2. Aggregations and Metrics**  
- The use of `MIN`, `MAX`, `AVG`, and `SUM` for CPU, memory, and network metrics provides a comprehensive view of resource utilization.
- Calculating both used and unused percentages for CPU and memory helps identify inefficiencies and potential optimization opportunities.

**3. Filtering and Partition Pruning**  
- The query filters on `DATE(pu.period_start_time) = DATE_ADD(CURRENT_DATE(), -1)` and `pu.result_state = 'COMPLETED'`, which enables partition pruning if the underlying tables are partitioned by date or result state. This is a best practice for query performance[[2]](https://docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/ "/docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/").

**4. Use of Managed and System Tables**  
- The query leverages Databricks system tables, which are managed and optimized for analytics and observability use cases[[1]](https://docs.databricks.com/aws/en/data-engineering/best-practices/ "/docs.databricks.com/aws/en/data-engineering/best-practices/").

**5. Cost Analysis**  
- Joining with the billing usage table and aggregating cost per pipeline run is a recommended approach for cost observability and optimization[[1]](https://docs.databricks.com/aws/en/data-engineering/best-practices/ "/docs.databricks.com/aws/en/data-engineering/best-practices/").

**6. Data Preparation and Star Schema**  
- The query structure resembles a star schema, with the pipeline run as the fact and the other tables as dimensions, which is a best practice for analytical queries[[2]](https://docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/ "/docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/").

### Optimization Recommendations

- **Statistics and Caching:**  
  Ensure that statistics are up to date on all system tables. If you notice slow performance, consider running `ANALYZE TABLE <table> COMPUTE STATISTICS FOR ALL COLUMNS` on large tables to help the cost-based optimizer make better decisions[[3]](https://docs.databricks.com/aws/en/optimizations/cbo/ "/docs.databricks.com/aws/en/optimizations/cbo/").
- **Column Selection:**  
  Only select columns you need for your analysis to reduce data shuffling and improve performance.
- **Materialized Views:**  
  If you run this query frequently, consider creating a materialized view or a persisted table for the aggregated results to further improve performance for downstream queries[[2]](https://docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/ "/docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/").
- **Predicate Pushdown:**  
  Make sure your filters (especially on date and result state) are applied as early as possible in the query to take advantage of predicate pushdown and partition pruning[[2]](https://docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/ "/docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/").

### Final Confirmation

Your query is well-aligned with Databricks best practices for data engineering, observability, and cost analysis. It is optimized for performance and clarity, and it leverages the strengths of the Databricks platform and system tables. For further optimization, keep your system tables' statistics up to date and consider materialized views for repeated analysis[[3]](https://docs.databricks.com/aws/en/optimizations/cbo/ "/docs.databricks.com/aws/en/optimizations/cbo/")[[2]](https://docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/ "/docs.databricks.com/aws/en/cheat-sheet/bi-serving-data-prep/")[[1]](https://docs.databricks.com/aws/en/data-engineering/best-practices/ "/docs.databricks.com/aws/en/data-engineering/best-practices/").

If you have very large data volumes or performance concerns, you can use the `EXPLAIN` command to review the query plan and ensure that joins and aggregations are being executed efficiently. If statistics are missing, update them as recommended by the optimizer[[3]](https://docs.databricks.com/aws/en/optimizations/cbo/ "/docs.databricks.com/aws/en/optimizations/cbo/").
...
To convert your SQL query into Python code using pandas, you would first need to read the relevant Parquet files from S3 into pandas DataFrames. Then, you would perform the necessary joins and aggregations using pandas operations.
...
# Read each table from its respective Parquet file in S3
pipeline_update_timeline = pd.read_parquet('s3://your-bucket/system.lakeflow.pipeline_update_timeline.parquet')
pipelines = pd.read_parquet('s3://your-bucket/system.lakeflow.pipelines.parquet')
clusters = pd.read_parquet('s3://your-bucket/system.compute.clusters.parquet')
node_types = pd.read_parquet('s3://your-bucket/system.compute.node_types.parquet')
node_timeline = pd.read_parquet('s3://your-bucket/system.compute.node_timeline.parquet')
billing_usage = pd.read_parquet('s3://your-bucket/system.billing.usage.parquet')

# Filter for yesterday and completed runs
yesterday = pd.Timestamp.today().normalize() - pd.Timedelta(days=1)
pipeline_update_timeline = pipeline_update_timeline[
    (pipeline_update_timeline['result_state'] == 'COMPLETED') &
    (pd.to_datetime(pipeline_update_timeline['period_start_time']).dt.date == yesterday.date())
]

pipelines = pipelines[pipelines['pipeline_type'] == 'ETL_PIPELINE']

# Merge/join tables
merged = pipeline_update_timeline.merge(
    pipelines, on=['workspace_id', 'pipeline_id'], how='inner'
).merge(
    clusters, left_on='compute', right_on='cluster_id', how='inner'
).merge(
    node_timeline, left_on='cluster_id', right_on='cluster_id', how='inner'
)

# Join node_types for worker nodes
merged = merged.merge(
    node_types, left_on='worker_node_type', right_on='node_type', how='left'
)

# Join billing usage
merged = merged.merge(
    billing_usage,
    left_on=['cluster_id', 'pipeline_id', 'update_id'],
    right_on=['usage_metadata.cluster_id', 'usage_metadata.dlt_pipeline_id', 'usage_metadata.dlt_update_id'],
    how='left'
)

# Aggregations per pipeline run
agg = merged.groupby(['workspace_id', 'pipeline_id', 'update_id']).agg(
    min_cpu_used_percent=pd.NamedAgg(column='cpu_user_percent', aggfunc='min'),
    max_cpu_used_percent=pd.NamedAgg(column='cpu_user_percent', aggfunc='max'),
    avg_cpu_used_percent=pd.NamedAgg(column='cpu_user_percent', aggfunc='mean'),
    min_mem_used_percent=pd.NamedAgg(column='mem_used_percent', aggfunc='min'),
    max_mem_used_percent=pd.NamedAgg(column='mem_used_percent', aggfunc='max'),
    avg_mem_used_percent=pd.NamedAgg(column='mem_used_percent', aggfunc='mean'),
    total_network_sent_mb=pd.NamedAgg(column='network_sent_bytes', aggfunc=lambda x: x.sum() / (1024 * 1024)),
    total_network_received_mb=pd.NamedAgg(column='network_received_bytes', aggfunc=lambda x: x.sum() / (1024 * 1024)),
    total_cost=pd.NamedAgg(column='usage_amount', aggfunc='sum')
).reset_index()

print(agg)
```
This code:
- Reads each table from S3 Parquet files.
- Filters for completed pipeline runs for yesterday.
- Joins the tables on relevant keys.
- Aggregates CPU, memory, network, and cost metrics per pipeline run.

**Note:**  
- You may need to adjust column names and join keys to match your actual schema.
- If your Parquet files are partitioned, you can optimize reads by specifying filters in `read_parquet`.
- For large datasets, consider using PySpark DataFrames instead of pandas for scalability.

Let me know if you need a PySpark version or want to see how to write the results back to S3!
...
WITH node_metrics AS (
  SELECT
    ...
    nt.core_count,
    nt.memory_mb,
    -- Derived Absolute Metrics
    ((ntl.cpu_user_percent + ntl.cpu_system_percent) / 100.0) * nt.core_count AS actual_cpu_cores_used,
    (ntl.mem_used_percent / 100.0) * nt.memory_mb AS actual_mem_mb_used,
    -- Telemetry
    ntl.cpu_user_percent,
    ntl.cpu_system_percent,
    ntl.cpu_wait_percent,
    ntl.mem_used_percent,
    ntl.mem_swap_percent,
    ntl.network_transmit_bytes,
    ntl.network_receive_bytes
  FROM system.compute.node_timeline ntl
  JOIN system.compute.clusters c ON ntl.cluster_id = c.cluster_id
  JOIN system.compute.node_types nt ON 
    (ntl.driver = true AND c.driver_node_type_id = nt.node_type_id) OR 
...
SELECT
    ...
    put.update_id,
    nm.driver AS is_driver,
    -- Under-Utilization Logic
    CASE 
        WHEN MAX(nm.cpu_user_percent + nm.cpu_system_percent) < 15 AND MAX(nm.mem_used_percent) < 30 
        THEN 'UNDERUTILIZED (Over-Provisioned)'
        WHEN MAX(nm.cpu_user_percent + nm.cpu_system_percent) > 85 
        THEN 'HIGH LOAD (Correctly Sized or Under-Sized)'
        ELSE 'OPTIMIZED'
    END AS resource_efficiency_check,
    
    -- CPU Stats
    ROUND(AVG(nm.actual_cpu_cores_used), 2) AS avg_cpu_cores_active,
    MAX(nm.cpu_user_percent + nm.cpu_system_percent) AS max_total_cpu_pct,
    AVG(nm.cpu_wait_percent) AS avg_cpu_wait_pct,
    
    -- Memory Stats
    ROUND(AVG(nm.actual_mem_mb_used), 2) AS avg_mem_mb_used,
    MAX(nm.mem_used_percent) AS max_mem_used_pct,
    MAX(nm.mem_swap_percent) AS max_swap_pct,
    
    -- Network Totals
    SUM(nm.network_transmit_bytes) AS total_sent_bytes,
    SUM(nm.network_receive_bytes) AS total_received_bytes
...
usage_start_time (NOT NULL) -- The start time for this usage record.
usage_start_time >= price_start_time
usage_start_time < price_end_time
usage_end_time (NOT NULL) -- The end time for this usage record.
usage_date (NOT NULL) -- Date of the usage record, this field can be used for faster aggregation by date.
usage_quantity (NOT NULL) != 0 -- Having number of the units consumed for this usage
SUM(usage_quantity * pricing.effective_list.default) AS total_dbu_cost_usd
billing_origin_product (NOT NULL) -- The product that originated the usage. See https://docs.databricks.com/aws/en/admin/system-tables/billing#billing-origin-product-reference
usage_type (NOT NULL) -- The type of usage attributed to the product or workload for billing purposes.
product_features (NOT NULL) -- Details about the specific product features used. See https://docs.databricks.com/aws/en/admin/system-tables/billing#features
ingestion_date (NOT NULL) -- Date the record loaded into the usage table.

-- The values in usage_metadata are all strings that tell about the workspace objects and resources involved in the usage.
-- NOTE: Depending on the compute type and features used, only a subset of these values is populated in any given usage. 
usage_metadata (NOT NULL) -- System-provided metadata reference about the usage, including IDs for compute resources i.e., cluster id, node type, dlt_pipeline_id. See https://docs.databricks.com/aws/en/admin/system-tables/billing#usage-metadata-reference
usage_metadata.cluster_id (NOT NULL) -- ID of the cluster associated with the usage.
usage_metadata.node_type (NOT NULL) -- Instance type of the compute resource.
usage_metadata.dlt_pipeline_id? -- ID of the declarative pipeline associated with the usage.
usage_metadata.dlt_update_id AS billing_update_id? -- ID of the pipeline update associated with the usage.

-- Pricing info at the published list price rate. The price rates may be changed between months. 
-- NOTE: For cost estimation, must NOT include any promotional represents a temporary promotional price for cost estimation.
price_start_time (NOT NULL) -- The time this price became effective.
price_end_time (NOT NULL) -- The time this price stopped being effective.
pricing.default (NOT NULL) AS default_price -- The default price always a single price that can be used for long-term estimates.
pricing.effective_list.default (NOT NULL) AS effective_price -- The effective list price used for calculating the cost. 
...
Pipeline metadata
update_id AS pipeline_update_id
result_state = 'COMPLETED'
cluster_id AS pipeline_cluster_id
pipeline_id
pipeline_type = 'ETL_PIPELINE'


Add a join to system.billing.usage and system.billing.list_prices that billing_origin_product is ETL specific to 
calculate exactly how much money is being "wasted" on these underutilized runs?

To calculate the financial impact of over-provisioning, join the billing and pricing system tables with the compute metrics/telemetrics. 
In the logic below, "Wasted Cost" is defined as the proportional cost incurred when a pipeline never exceeds a 
"Safe Peak" (defined here as 25% CPU or 40% Memory). If a pipeline stays below these thresholds, calculate the percentage of 
the DBU cost that was spent on idle headroom.

ETL Pipeline: Wasted Cost Analysis (Under-Utilization Audit)

WITH pipeline_billing AS (
  -- Aggregating ETL/DLT costs per update
  SELECT 
  ...
  JOIN system.billing.list_prices lp 
    ON u.sku_name = lp.sku_name
  WHERE u.billing_origin_product = 'DLT' -- DLT is the product origin for ETL Pipelines
    AND u.usage_metadata.dlt_update_id IS NOT NULL
    AND u.usage_start_time >= lp.price_start_time 
    AND (lp.price_end_time IS NULL OR u.usage_start_time < lp.price_end_time)
  GROUP BY ALL
),
node_telemetry AS (
  -- Aggregating peak utilization per node role per update
  SELECT
    ntl.cluster_id,
    ntl.driver,
    MAX(ntl.cpu_user_percent + ntl.cpu_system_percent) AS max_cpu_util_pct,
    MAX(ntl.mem_used_percent) AS max_mem_util_pct,
    AVG(ntl.cpu_wait_percent) AS avg_cpu_wait_pct
  FROM system.compute.node_timeline ntl
  GROUP BY ALL
)
SELECT
    ...
    -- Financials
    ROUND(pb.total_dbus, 2) AS dbus_consumed,
    ROUND(pb.total_cost_usd, 2) AS actual_cost_usd,
    
    -- Peak Utilization
    MAX(CASE WHEN nt.driver THEN nt.max_cpu_util_pct END) AS driver_peak_cpu_pct,
    MAX(CASE WHEN NOT nt.driver THEN nt.max_cpu_util_pct END) AS worker_peak_cpu_pct,
    
    -- Waste Calculation Logic
    -- If Peak CPU < 25% and Peak Mem < 40%, we estimate 50% of the cost is "Waste" 
    -- due to over-provisioned instance sizing.
    CASE 
      WHEN MAX(nt.max_cpu_util_pct) < 25 AND MAX(nt.max_mem_util_pct) < 40 
      THEN ROUND(pb.total_cost_usd * 0.50, 2) 
      ELSE 0 
    END AS estimated_waste_usd,

    CASE 
      WHEN MAX(nt.max_cpu_util_pct) < 25 AND MAX(nt.max_mem_util_pct) < 40 THEN 'CRITICAL: OVER-PROVISIONED'
      WHEN MAX(nt.max_cpu_util_pct) < 10 THEN 'GHOST RUN: NEAR ZERO UTILIZATION'
      ELSE 'OPTIMIZED'
    END AS efficiency_status

FROM system.lakeflow.pipeline_update_timeline put
JOIN system.lakeflow.pipelines p ON put.pipeline_id = p.pipeline_id
JOIN system.compute.clusters c ON put.cluster_id = c.cluster_id
JOIN pipeline_billing pb ON put.update_id = pb.update_id
JOIN node_telemetry nt ON put.cluster_id = nt.cluster_id

WHERE put.result_state = 'COMPLETED'
  AND p.pipeline_type = 'ETL_PIPELINE'
  
GROUP BY ALL
HAVING estimated_waste_usd > 0
ORDER BY estimated_waste_usd DESC;

Technical Breakdown of the "Waste" Check 
	1	Product Alignment: Use billing_origin_product = 'DLT' because all Lakeflow/ETL pipeline activity under the DLT product family.
	2	The Over-Provisioning Threshold:
	◦	Max CPU < 25%: If the application code never stresses more than a quarter of the available cores, the instance is too large.
	◦	Max Memory < 40%: Databricks/Spark requires headroom for overhead, but if 60% of compute RAM is untouched, result paying for "ghost" memory.
	3	Wasted USD Calculation:
	◦	The query applies a conservative 50% waste multiplier to the total cost for runs that fail both thresholds. This represents the potential 
savings if you were to downsize the instance type (e.g., from an m5.4xlarge to an m5.2xlarge).
	4	Ghost Runs: The status GHOST RUN identifies pipelines that triggered, spun up a cluster, and finished with almost no work done—often 
caused by scheduled triggers on empty source folders. 

Recommendation for Next Steps 
	•	Right-sizing: For pipelines flagged as CRITICAL, look at the worker_node_type_id and move one "size" down (e.g., Standard_DS4_v2 to Standard_DS3_v2).
	•	Autoscale Tuning: If the cost is high but the worker peak is low, check if your min_workers is set too high, forcing the cluster to stay large even when the workload is light. 
Would you like me to aggregate this by pipeline_name over the last 30 days to show you the total monthly "Lost Dollars" per project?
...
To analyze the association between Lakeflow Declarative Pipelines (formerly DLT) pipeline update timelines and billing, pricing, and compute resource utilization, you need to join several Databricks system tables:

1. **Pipeline Update Timeline**:  
   - Table: `system.lakeflow.pipeline_update_timeline`
   - Contains metadata for each pipeline update, including `pipeline_id`, `update_id`, `compute` (details about compute resources), and timestamps[[1]](https://docs.databricks.com/aws/en/admin/system-tables/jobs/ "/docs.databricks.com/aws/en/admin/system-tables/jobs/").

2. **Billing Usage**:  
   - Table: `system.billing.usage`
   - Contains billing records, including `usage_metadata.dlt_pipeline_id` (pipeline ID), `sku_name`, `usage_date`, `usage_quantity` (DBU consumption), and compute SKU information[[2]](https://docs.databricks.com/aws/en/admin/usage/system-tables/ "/docs.databricks.com/aws/en/admin/usage/system-tables/")[[3]](https://docs.databricks.com/aws/en/ingestion/lakeflow-connect/monitor-costs/ "/docs.databricks.com/aws/en/ingestion/lakeflow-connect/monitor-costs/")[[4]](https://docs.databricks.com/aws/en/ldp/monitor-event-logs/ "/docs.databricks.com/aws/en/ldp/monitor-event-logs/").

3. **Billing List Prices (Cost Estimates)**:  
   - Table: `system.billing.list_prices`
   - Contains SKU pricing information, which can be joined on `sku_name` to estimate costs.

4. **Compute Metadata**:  
   - The `compute` column in `system.lakeflow.pipeline_update_timeline` contains details about the compute resources used, such as node types, clusters, and node timeline[[1]](https://docs.databricks.com/aws/en/admin/system-tables/jobs/ "/docs.databricks.com/aws/en/admin/system-tables/jobs/").

### Example SQL Query

Here’s a template to join these tables and retrieve the desired information for each pipeline update:
...
SELECT
  ...
  t.period_start_time,
  t.period_end_time,
  t.compute.node_types AS node_types,
  t.compute.clusters AS clusters,
  t.compute.node_timeline AS node_timeline,
  b.sku_name,
  b.usage_date,
  SUM(b.usage_quantity) AS dbu_usage,
  p.list_price AS sku_list_price,
  SUM(b.usage_quantity * p.list_price) AS estimated_cost
FROM system.lakeflow.pipeline_update_timeline t
LEFT JOIN system.billing.usage b
  ON b.usage_metadata.dlt_pipeline_id = t.pipeline_id
  AND b.usage_start_time >= t.period_start_time
  AND b.usage_end_time <= t.period_end_time
...
GROUP BY
  ...
  t.period_start_time,
  t.period_end_time,
  t.compute.node_types,
  t.compute.clusters,
  t.compute.node_timeline,
  b.sku_name,
  b.usage_date,
  p.list_price
ORDER BY t.period_start_time DESC
```
- This query links each pipeline update to its billing usage, cost estimates, and compute metadata.
- Adjust the date range and pipeline IDs as needed for your analysis.

**Notes:**
- The `compute` struct in the timeline table provides detailed information about the compute resources used for each update, including node types and cluster configuration[[1]](https://docs.databricks.com/aws/en/admin/system-tables/jobs/ "/docs.databricks.com/aws/en/admin/system-tables/jobs/").
- Billing usage is tracked per pipeline update and can be aggregated by date, SKU, or other dimensions[[2]](https://docs.databricks.com/aws/en/admin/usage/system-tables/ "/docs.databricks.com/aws/en/admin/usage/system-tables/")[[3]](https://docs.databricks.com/aws/en/ingestion/lakeflow-connect/monitor-costs/ "/docs.databricks.com/aws/en/ingestion/lakeflow-connect/monitor-costs/")[[4]](https://docs.databricks.com/aws/en/ldp/monitor-event-logs/ "/docs.databricks.com/aws/en/ldp/monitor-event-logs/").
- List prices are estimates; actual costs may vary based on negotiated rates or discounts.

**References:**  
- [Monitor costs using system tables][[2]](https://docs.databricks.com/aws/en/admin/usage/system-tables/ "/docs.databricks.com/aws/en/admin/usage/system-tables/")
- [Monitor managed ingestion pipeline cost][[3]](https://docs.databricks.com/aws/en/ingestion/lakeflow-connect/monitor-costs/ "/docs.databricks.com/aws/en/ingestion/lakeflow-connect/monitor-costs/")
- [Pipeline event log: Query the cost of a pipeline update][[4]](https://docs.databricks.com/aws/en/ldp/monitor-event-logs/ "/docs.databricks.com/aws/en/ldp/monitor-event-logs/")
- [Pipeline update timeline table schema][[1]](https://docs.databricks.com/aws/en/admin/system-tables/jobs/ "/docs.databricks.com/aws/en/admin/system-tables/jobs/")
